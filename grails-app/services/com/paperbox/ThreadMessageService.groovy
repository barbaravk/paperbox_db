...
class ThreadMessageService {
    ...
    Message sendMessage(User from, User to, String text, String subject) {
        return sendThreadMessage(from.id, to.id, from.name, to.name, text, subject)
    }
     ...
     * @return a map with: messages (the list of Messages), totalNum (the total num of messages, for the pagination), unreadNum (the number of unreaded messages)
     */
    Map getAllByThread(long id, int offset = 0, int itemsByPage = -1, String sort='dateCreated', String order='asc'){
        ...
        result.unreadNum = resultMessages.count{ it.readed == false }
