 ...
 */
// FileName: manifests/listing.js
//= require galleryinput.js
//= require_tree ../final_tiles
...
$(document).ready(function(){
    ...
    $('#listingItemAdd').on('click', function() {
        var $container = $('.item-add-container')
        var $new_line = $('.item-line:first').clone()
        $new_line.find('h6').remove()
        $new_line.find('input').val('')
        //$new_line.find('.item-name').attr('name', 'itemName_' + ($('.item-line').length+1))
        //$new_line.find('.item-desc').attr('name', 'itemDesc_' + ($('.item-line').length+1))
        //$new_line.find('.item-price').attr('name', 'itemPrice_' + ($('.item-line').length+1))
        ...
        if (items.length > 0) {
            items.each(function(index){
                if ($(this).find('.item-name').val() != '') {
                    ...
                    var itemDescription =  $(this).find('.item-desc').val()
                    var itemPrice =  $(this).find('.item-price').val()
                    var itemData = {'itemName': itemName, 'itemDescription': itemDescription, 'itemPrice': itemPrice}
                    itemsObject['item_'+i] = itemData
                }
            })
        ...
        if (tiles.length > 0) {
            tiles.each(function (index) {
                ...
                if (image) {
                    ...
                    formData.append('imageData_'+i, blob)
                    var title = $(this).find('.title.edited').text()
                    var subtitle = $(this).find('.subtitle.edited').text()
                    tilesObject['image_' + i] = {'title': title, 'subtitle': subtitle}
                }
            })
            var tilesJson = JSON.stringify(tilesObject)
            formData.append('images', tilesJson)
        }

        $.ajax({
            url: '/listing/saveListingAjax',
            type: 'POST',
            data: formData,
    ...
    function imageByteString (dataURI) {
        'use strict'
        var byteString
