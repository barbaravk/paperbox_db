...
//    def mailService
    ...
    def inbox() {
        ...
        if (params.sort == 'fromName') {
            ...
            //If the user hadn't ask for a sort, make a special sort with the unread messages first
            sort = 'readed'
        }
        ...
        render view:'inbox', model:[user:currentUser, messages:result.messages, totalNum:result.totalNum, unreadNum:result.unreadNum, max:ITEMS_BY_PAGE, sort:sort, order:order]
    }
        ...
        if (toUser) {
            ...
            if (!userBlock) {
                if (params.subject && params.text && params.text.size()<=5000) {
                    threadMessageService.sendMessage(currentUser, toUser, params.text, params.subject)
...
//            threadMessageService.sendMessage(currentUser, admin, text, message(code: 'report.subject'))
//
